...
class DeleteFlagModel(models.Model):
    ...
    deleted_at = models.DateTimeField(
        auto_now=True, null=True, blank=True, verbose_name="Дата удаления"
    )
    deleted_user = models.ForeignKey(
        User, on_delete=models.CASCADE, null=True, blank=True, verbose_name="Удалил"
    )
...
class BaseScheduleModel(models.Model):
    day_of_week = models.CharField(
        max_length=30, choices=DayOfWeekChoices.choices, verbose_name="День недели"
    )
    start_time = models.TimeField(default=time(hour=0, minute=0), verbose_name="Время начала")
    end_time = models.TimeField(default=time(hour=23, minute=59), verbose_name="Время окончания")
